              ...
              {activeTab === "profile" && (
                <div className="space-y-8">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setIsEditing(!isEditing)} className="text-sm font-semibold text-cyan-400 hover:text-cyan-300">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        <span className="rounded-full bg-purple-500/10 px-2 py-0.5 text-[10px] font-bold text-purple-400 border border-purple-500/20">PRO MEMBER</span>
                        <span className="rounded-full bg-red-500/10 px-2 py-0.5 text-[10px] font-bold text-red-400 border border-red-500/20">{user.stats.riskLevel.toUpperCase()}</span>
                      </div>
                    </div>
                  </div>
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <input disabled={!isEditing} type="text" value={user.displayName} onChange={(e) => setUser({ ...user, displayName: e.target.value })} className="w-full rounded-xl bg-black/20 border border-white/10 px-4 py-3 text-white focus:border-cyan-500 focus:outline-none disabled:opacity-50" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-bold uppercase text-slate-500">Email Address</label>
                      <input disabled={!isEditing} type="email" value={user.email} className="w-full rounded-xl bg-black/20 border border-white/10 px-4 py-3 text-white focus:border-cyan-500 focus:outline-none disabled:opacity-50" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase text-slate-500">Bio</label>
                    <textarea disabled={!isEditing} rows={3} value={user.bio} className="w-full rounded-xl bg-black/20 border border-white/10 px-4 py-3 text-white focus:border-cyan-500 focus:outline-none disabled:opacity-50" />
                  </div>
                  <div className="space-y-4 pt-4 border-t border-white/5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {isEditing && (
                        <button className="rounded-full border border-dashed border-slate-600 px-3 py-1.5 text-sm text-slate-500 hover:text-white hover:border-white transition-colors">+ Add Ticker</button>
                      )}
                    </div>
                  </div>
                </div>
